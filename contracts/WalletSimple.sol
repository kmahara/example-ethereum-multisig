...
contract WalletSimple {
  ...
  event Error(address from, address to, address otherSigner, uint value, bytes data);
  ...
  function sendMultiSig(
    ...
    if (!success) {
      emit Error(msg.sender, toAddress, otherSigner, value, data);
      revert(string(abi.encodePacked("execute failed: value=", uintToString(value))));
    }
  ...
  function verifyMultiSig(
    ...
    if (!isSigner(otherSigner)) {
      ...
      revert("otherSigner is not signer");
    }
    if (otherSigner == msg.sender) {
      // Cannot approve own transaction
      revert("otherSigner must not be message sender");
    }
